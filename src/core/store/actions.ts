...
import { IEmployee, IEmployeeData, ISelectOptionProps, ITableProps } from "../interfaces/interface.ts";
...
export const fetchDropdownData = () => {
  return function (dispatch: Dispatch) {
      ...
      getData(apiURL.skills)
        .then((reponse) => dispatch(setSkills(transformArrayToOptionsList(reponse.data.data))))
        .catch((error) => {
          toast.error("Skills could not be fetched.");
          console.error("Error fetching dropdown data:", error);
        });
      getData(apiURL.roles)
        .then((reponse) => dispatch(setRoles(transformArrayToOptionsList(reponse.data))))
        .catch((error) => {
